            ...
            if 'status' in claim_daily:
                if claim_daily['status'] == 0:
                    return self.print_timestamp(
                        f"{Fore.BLUE + Style.BRIGHT}[ {first_name} ]{Style.RESET_ALL}"
                        f"{Fore.WHITE + Style.BRIGHT} | {Style.RESET_ALL}"
                    ...
                    return self.print_timestamp(
                        f"{Fore.BLUE + Style.BRIGHT}[ {first_name} ]{Style.RESET_ALL}"
                        f"{Fore.WHITE + Style.BRIGHT} | {Style.RESET_ALL}"
                        f"{Fore.GREEN + Style.BRIGHT}[ Already Check Daily Claim ]{Style.RESET_ALL}"
                    )
                    ...
                    if data_rank['data']['isCreated']:
                        if data_rank['data']['unusedStars'] == 0:
                            return self.print_timestamp(
                                f"{Fore.BLUE + Style.BRIGHT}[ {first_name} ]{Style.RESET_ALL}"
                                f"{Fore.WHITE + Style.BRIGHT} | {Style.RESET_ALL}"
                                f"{Fore.GREEN + Style.BRIGHT}[ Rank {data_rank['data']['currentRank']['name']} ]{Style.RESET_ALL}"
                            )
        ...
        try:
            ...
            if 'status' in evaluate_rank:
                if evaluate_rank['status'] == 0:
                    ...
                    return self.print_timestamp(
                        f"{Fore.GREEN + Style.BRIGHT}[ {first_name} ]{Style.RESET_ALL}"
                        f"{Fore.WHITE + Style.BRIGHT} | {Style.RESET_ALL}"
                    ...
                    if create_rank['data']['isCreated']:
                        if create_rank['data']['unusedStars'] == 0:
                            return self.print_timestamp(
                                ...
                                f"{Fore.WHITE + Style.BRIGHT} | {Style.RESET_ALL}"
                                f"{Fore.GREEN + Style.BRIGHT}[ Rank {create_rank['data']['currentRank']['name']} ]{Style.RESET_ALL}"
                            )
                        ...
                        if balance['balance_type'] == 'Star':
                            if balance['balance'] == 0:
                                return self.print_timestamp(
                                    f"{Fore.BLUE + Style.BRIGHT}[ {first_name} ]{Style.RESET_ALL}"
                                    f"{Fore.WHITE + Style.BRIGHT} | {Style.RESET_ALL}"
                                    f"{Fore.GREEN + Style.BRIGHT}[ You Did Not Have Tomarket Star ]{Style.RESET_ALL}"
                                )
                            else:
        ...
        while True:
            try:
                ...
                for account in accounts:
                    ...
                    self.print_timestamp(
                        f"{Fore.BLUE + Style.BRIGHT}[ {account['first_name']} ]{Style.RESET_ALL}"
                        f"{Fore.WHITE + Style.BRIGHT} | {Style.RESET_ALL}"
                    ...
                    if 'farming' in balance['data']:
                        if datetime.now().astimezone() >= datetime.fromtimestamp(balance['data']['farming']['end_at']).astimezone():
                            ...
                            self.print_timestamp(
                                f"{Fore.BLUE + Style.BRIGHT}[ {account['first_name']} ]{Style.RESET_ALL}"
                                f"{Fore.WHITE + Style.BRIGHT} | {Style.RESET_ALL}"
                                f"{Fore.BLUE + Style.BRIGHT}[ Farm Can Be Claim At {datetime.fromtimestamp(balance['data']['farming']['end_at']).astimezone().strftime('%X %Z')} ]{Style.RESET_ALL}"
                            )
                    else:
                ...
                self.print_timestamp(
                    f"{Fore.GREEN + Style.BRIGHT}[ Total Account {len(accounts)} ]{Style.RESET_ALL}"
                    f"{Fore.WHITE + Style.BRIGHT} | {Style.RESET_ALL}"
                    f"{Fore.GREEN + Style.BRIGHT}[ Total Balance {total_balance} ]{Style.RESET_ALL}"
                )
                self.print_timestamp(f"{Fore.BLACK + Style.BRIGHT}[ Restarting At {(datetime.now().astimezone() + timedelta(seconds=sleep_time)).strftime('%X %Z')} ]{Style.RESET_ALL}")
...
if __name__ == '__main__':
    try:
        ...
        tomarket.print_timestamp(f"{Fore.WHITE + Style.BRIGHT}[ Select an option ]{Style.RESET_ALL}")
        tomarket.print_timestamp(
            f"{Fore.WHITE + Style.BRIGHT}[ 1 ]{Style.RESET_ALL}"
            f"{Fore.WHITE + Style.BRIGHT} | {Style.RESET_ALL}"
            f"{Fore.GREEN + Style.BRIGHT}[ Generate Tokens ]{Style.RESET_ALL}"
        )
        tomarket.print_timestamp(
            f"{Fore.BLUE + Style.BRIGHT}[ 2 ]{Style.RESET_ALL}"
            f"{Fore.WHITE + Style.BRIGHT} | {Style.RESET_ALL}"
            f"{Fore.GREEN + Style.BRIGHT}[ Account Ready > ]{Style.RESET_ALL}"
        )

        initial_choice = int(input(
            f"{Fore.WHITE + Style.BRIGHT}[ Enter The Number ]{Style.RESET_ALL}"
            f"{Fore.WHITE + Style.BRIGHT} | {Style.RESET_ALL}"
        ))
        if initial_choice == 1:
            tomarket.print_timestamp(f"{Fore.BLUE + Style.BRIGHT}[ Generate Tokens ]{Style.RESET_ALL}")
            tomarket.process_queries()
            tomarket.print_timestamp(f"{Fore.GREEN + Style.BRIGHT}[ Generation Completed ]{Style.RESET_ALL}")
        ...
        tomarket.print_timestamp(f"{Fore.WHITE + Style.BRIGHT}[ Select The Accounts ]{Style.RESET_ALL}")
        for i, accounts_file in enumerate(account_files, start=1):
            tomarket.print_timestamp(
                f"{Fore.BLUE + Style.BRIGHT}[ {i} ]{Style.RESET_ALL}"
                f"{Fore.WHITE + Style.BRIGHT} | {Style.RESET_ALL}"
                f"{Fore.BLACK + Style.BRIGHT}[ {accounts_file} ]{Style.RESET_ALL}"
            )
